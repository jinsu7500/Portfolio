...
+ 개발이력
  ...
  + [(2021) 2D 실시간 협동 퍼즐게임 BreakFriendShip 개발 팀리더](#BreakFriendShip)
   ...
   <금융>
  + (2022) (주)파이낸셜데이타시스템 - 증권수탁패키지(G-evis 3.0) 고도화 개발참여
  + (2022) (주)파이낸셜데이타시스템 - 농협은행 증권수탁 디지털화 고도화 프로젝트 개발참여 (수탁 실물 담당 F/O,B/O 개발)
  + (2023) (주)파이낸셜데이타시스템 - 우리은행 신탁트레저리 재구축 프로젝트 개발참여 (회계/실물/자금, F/O, B/O,EAI 계정계 연계 시스템 개발)
